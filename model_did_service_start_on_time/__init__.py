...
from brainer.schemas_df.features import (
    FeaturesServiceDidServiceStartOnTimePredictDFSchema,
    FeaturesServiceDidServiceStartOnTimeTrainDFSchema,
)


class ModelDidServiceStartOnTime(BaseCatBoostModel, metaclass=Singleton):
    """Class for CatBoost model did_service_start_on_time."""
    ...
    def __init__(self):
        ...
        super().__init__(
            target="did_service_start_on_time",
    ...
    @check_input(FeaturesServiceDidServiceStartOnTimeTrainDFSchema.to_schema(), "df")
    def preprocess_train(
        self,
        *,
        df: DataFrame[FeaturesServiceDidServiceStartOnTimeTrainDFSchema],
        test_size: float = 0.2,
    ...
    @check_input(FeaturesServiceDidServiceStartOnTimePredictDFSchema.to_schema())
    def preprocess_predict(
        self,
        df: DataFrame[FeaturesServiceDidServiceStartOnTimePredictDFSchema],
    ) -> pd.DataFrame:
        """Preprocess the data.
